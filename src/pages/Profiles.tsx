...
export function Profiles() {
  ...
  const loadTransforms = async () => {
    setTransformsRefreshing(true);
    try {
      const h = await runLuma(["transforms", "list"], false);
      const r = await h.done;
      setTransforms((r.payload as { transforms?: Transform[] })?.transforms ?? []);
    } finally {
      setTransformsRefreshing(false);
    }
  };
